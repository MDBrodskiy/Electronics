  ...
  \item We begin by constructing the circuit:
    ...
    \begin{enumerate}
      ...
      \item Running an AC Simulation, we obtain the following:
        ...
        Note that critical values are labeled in the figure above.
      ...
      \item Running a transient simulation produces the following:
        ...
        Note that, in the above figure, the green sinusoid corresponds to the input and is shown with respect to vertical axis 1 (on the left). The red sinusoid represents the output voltage, and corresponds to the second vertical axis, on the right. Based on the graph, we observe that the \underline{input has a peak amplitude} of $2[\si{\milli\volt}]$, as expected, while \underline{the output has a peak amplitude} of $543[\si{\milli\volt}]$. Thus, we can calculate the gain as:
